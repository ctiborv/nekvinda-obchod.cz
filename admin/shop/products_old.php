...
-->
	...
	<tr>
		...
		<td class=\"tdright\">
			...
			<span class=\"f10i\">(odesláním fotografie přepíšete původní)</span></td>
	</tr>
...
function delete_img_good($good) {
	...
	$query = "SELECT img FROM ".T_GOODS." 
	WHERE id = $good AND ".SQL_C_LANG." LIMIT 0,1";
	$v = my_DB_QUERY($query,__LINE__,__FILE__);
	...
	@unlink(IMG_P_S.$img); // cesta pro nahled
	@unlink(IMG_P_M.$img); // cesta pro detail
	@unlink(IMG_P_O.$img); // cesta pro original
...
if(isset($_GET['a']) && ($_GET['a']=='edit' || $_GET['a']=='add') && !empty($_POST) && !isset($_POST['search']))
{
  ...
  { // novy zaznam
	...
	foreach($_POST['id_parent'] as $id_cat){
		...
		$query = "INSERT INTO ".T_GOODS_X_CATEGORIES." VALUES($id,$id_cat,'".C_LANG."')";
		my_DB_QUERY($query,__LINE__,__FILE__);
...
// 	my_DB_QUERY($query,__LINE__,__FILE__);
	...
	if(!empty($_FILES['foto']['name'])) {
		...
		$img = img_upload($nm=$id);
		
		$query = "UPDATE ".T_GOODS." SET img = '$img' WHERE id = $id AND ".SQL_C_LANG."";
		my_DB_QUERY($query,__LINE__,__FILE__);
	...
	if (!empty($_POST['id'])) {
  	...
  	$query = "DELETE FROM ".T_FOTO_ZBOZI." WHERE id_good = ".$id." ";
  	my_DB_QUERY($query,__LINE__,__FILE__);
		...
		if (!in_array("NO", $_POST['fotogalerie'])) {
  		...
  		while ($pV = each($_POST['fotogalerie'])) {
  			...
  			if(!empty($pV['value'])) {
  				...
  				$query = "INSERT INTO ".T_FOTO_ZBOZI." VALUES(0,$id,".$pV['value'].")";
  				my_DB_QUERY($query,__LINE__,__FILE__);
...
if(isset($_GET['a']) && ($_GET['a'] == "edit" || $_GET['a'] == "copy")) {
	...
	while ($z = mysql_fetch_assoc($v))
  {
    ...
    if($form_data['cpc_heureka'] == 0) $form_data['cpc_heureka'] = "";
		...
		if(!empty($z['img']) && $_GET['a'] != "copy") {
		
			$img = $_GET['id'].".".$z['img'];
			$img1 = IMG_P_S.$img; // cesta pro nahled
			$img2 = IMG_P_O.$img; // cesta pro detailni obrazek
			
			$form_data['img'] = showimg($img1,$img2,'','','','','',$timestamp);
			$form_data['img'] .= "<br /><a href=\"".MAIN_LINK."&f=products&delimg=".$_GET['id']."\" 
	...
	if ($_GET['id'] > 0) {
 	  /*echo $_GET['id'];
 	  exit;*/
   	$query = "SELECT id_kateg FROM ".T_FOTO_ZBOZI." 
   	WHERE id_good = ".$_GET['id']."";
   	$v = my_DB_QUERY($query,__LINE__,__FILE__);
   	while ($z = mysql_fetch_array($v)) {
  ...
  if(isset($var_selected_foto[$xid]))
  { // oznacene polozky
    $select_foto_item .= "<option value=\"$xid\" ".$var_selected_foto[$xid].">".$z['name']."</option>";
  }
  else
  { // neoznacene polozky
    $select_foto_item .= "<option value=\"$xid\">".$z['name']."</option>";
  }
	...
	if(!empty($form_data)) {
      ...
      $cat='';
		...
		// kategorie
		$form_data['id_parent'] = categories_checkbox($cat_array,$par_array,$dct,$parrents); // categories_select
...
if(isset($_GET['a']) && $_GET['a'] == "add") {
  ...
  $select_foto_item .= "<option value=\"$xid\" ".$var_selected_foto[$xid].">".$z['name']."</option>";
	...
	// kategorie
	$form_data['id_parent'] = categories_checkbox($cat_array,$_SESSION['last_id_parent'],$dct); // categories_select
...
else
{
	...
	// zrusime připojené fotogalerie
 	$query = "DELETE FROM ".T_FOTO_ZBOZI." WHERE id_good = ".$_GET['delete']." ";
	my_DB_QUERY($query,__LINE__,__FILE__);
